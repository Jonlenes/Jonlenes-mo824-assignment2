...
def subtourelim(n, model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        for var in model._vars:
            ...
            selected = gp.tuplelist((i, j) for i, j in var.keys() if vals[i, j] > 0.5)
            # find the shortest cycle in the selected edge list
            tour = subtour(selected, n)
            if len(tour) < n:
                # add subtour elimination constr. for every pair of cities in tour
                model.cbLazy(
                    gp.quicksum(var[i, j] for i, j in combinations(tour, 2))
                    <= len(tour) - 1
                )
...
def build_tsp_model(instance):
    ...
    # Objective
    model.setObjective(
        gp.quicksum(var_edges[i, j] * dist[i, j] for i, j in dist.keys()), GRB.MINIMIZE
    )
